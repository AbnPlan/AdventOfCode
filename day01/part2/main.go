...
func addNumbers(calibrationValue string) string {
	strNumsMap := make(map[int]rune)
	...
	for k, v := range numbers {
		...
		for {
			if !strings.Contains(temp, k) {
				break
			}

			keyIndex := strings.Index(temp, k)

			strNumsMap[keyIndex] = v
			temp = replaceAtIndex(temp, v, keyIndex)
		}
	}

	newStr := calibrationValue

	for k, v := range strNumsMap {
		newStr = replaceAtIndex(newStr, v, k)
	}

	return newStr
}

func processValues(calibrationValues []string) {
	for i := 0; i < len(calibrationValues); i++ {
		calibrationValues[i] = addNumbers(calibrationValues[i])
	}
}

func findIntegerPair(str string) int {
	var first rune = '-'
	var last rune = '-'

	for _, c := range str {
		if unicode.IsNumber(c) && first == '-' {
			first = c
...
func calibrate(calibrationValues []string) int {
	var counter int
	for _, s := range calibrationValues {
		counter += findIntegerPair(s)
	}
	return counter
}
...
func main() {
	...
	processValues(calibrationValues)

	fmt.Println(calibrate(calibrationValues))
}
