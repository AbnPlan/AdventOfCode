...
func separateID(puzzleInput []string) map[int]string {
	newMap:=make(map[int]string)
	for _, s := range puzzleInput{
		tempStr:=strings.Split(s, ":")
		ID, err:=strconv.Atoi(strings.Split(tempStr[0], " ")[1])
...
func separateGames(puzzleInput map[int]string) map[int][]string {
	newMap:=make(map[int][]string)

	for id, game := range puzzleInput {
		newMap[id] = strings.Split(game, ";")
	}

	return newMap
}

func separateColors(puzzleInput map[int][]string) map[int][][]string{
	newMap := make(map[int][][]string)

	for id, set := range puzzleInput {
		newMap[id] = make([][]string, 0)

		for _, v := range set{
			newMap[id] = append(newMap[id], strings.Split(v, ","))
		}
	}
...
func converToMap(puzzleInput map[int][][]string) map[int][]map[string]int {
	newMap:= make(map[int][]map[string]int)

	for id, sets := range puzzleInput {
		for _, set := range sets {
			tempMap := make(map[string]int)
			for _, entry := range set {
				splitKey := strings.Split(entry, " ")
				convertValue, _ := strconv.Atoi(splitKey[1])
				tempMap[splitKey[2]] = convertValue
			}
			newMap[id] = append(newMap[id], tempMap)
		}
	}
...
func processInput(input []string) map[int][]map[string]int {
	return converToMap(separateColors(separateGames(separateID(input))))
}

func verifyGame(game []map[string]int, maxBlue int, maxGreen int, maxRed int) bool {
	for _, roll:= range game {
		if roll["blue"] > maxBlue || roll["red"] > maxRed || roll["green"] > maxGreen{
			return false
		}
	}
	return true
...
func calculatePuzzle(input []string)int{
	puzzleMap:=processInput(input)
	counter:=0

	for id, game:= range puzzleMap {
		if verifyGame(game, 14, 13, 12){
			counter+=id
		}
	}
...
func main() {
	...
	fmt.Println(calculatePuzzle(puzzleInput))
}
