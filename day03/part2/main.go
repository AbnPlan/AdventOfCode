...
func findNumbers(input []string) map[string][]int {
	...
	for i, str := range input {
		...
		for j, char := range str {
			if unicode.IsDigit(char) {
				...
				buffer += string(char)
				starI, starJ:= checkForStar(i, j, input)
				if  starI != -1{
					tempStar = strconv.Itoa(starI) + strconv.Itoa(starJ)
					starFlag = true
				}
			} else {
				appendFlag = true
			...
			if (appendFlag || j == len(input[i]) - 1) && buffer != "" {
				if starFlag {
					tempNum, _ := strconv.Atoi(buffer)
					starMap[tempStar] = append(starMap[tempStar], tempNum)
				}
...
func checkForStar(i int, j int, input []string) (int, int) {
	lineIndex := [][]int{
		{i - 1, j - 1},
		{i - 1, j},
		{i - 1, j + 1},
	...
	for _, indexPair := range lineIndex {
		if indexPair[0] >= 0 && indexPair[1] >= 0 && indexPair[0] < len(input) && indexPair[1] < len(input[i]) {
			runeToCheck := rune(input[indexPair[0]][indexPair[1]])
			if runeToCheck == '*' {
				return indexPair[0] , indexPair[1]
			}
		}
	}
	return -1, -1
