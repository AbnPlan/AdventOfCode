...
func findNumbers(input []string) []int {
	...
	for i, str := range input {
		var buffer string
		var appendFlag bool
		var vecinityCheck bool

		for j, char := range str {
			if unicode.IsDigit(char) {
				appendFlag = false
				buffer += string(char)
				if passVecinity(i, j, input) {
					vecinityCheck = true
				}

				if j == len(input[i])-1 && buffer != "" {
					appendFlag = true
				}
			} else {
				appendFlag = true
			...
			if appendFlag && buffer != "" {
				if vecinityCheck {
					tempNum, _ := strconv.Atoi(buffer)
					numbers = append(numbers, tempNum)
				}
				buffer = ""
				vecinityCheck = false
			}
...
func passVecinity(i int, j int, input []string) bool {
	roundIndex := [][]int{
		{i - 1, j - 1},
		{i - 1, j},
		{i - 1, j + 1},
	...
	for _, indexPair := range roundIndex {
		if indexPair[0] >= 0 && indexPair[1] >= 0 && indexPair[0] < len(input) && indexPair[1] < len(input[i]) {
			...
			if runeToCheck != '.' {
				if i == indexPair[0] && unicode.IsDigit(runeToCheck) {
					continue
				}
				return true
			}
